...
*   The strength of competitor effects is also specified here. Demand for the
    ...
    *   By default, the parameter `competitor.demand.replacement` is defined as
        `list(loyalty = c(0.5, 0, 1))`. Thus, as the price of the advertiser's
        product price decreases and demand for the advertiser's brand increases,
        consumers conflicted between the advertiser and its competitors will
        choose the advertiser's brand at half the rate they would have in the
        absence of competitive pressure. For example, if, in the absence of the
        competition, 100% of switchers would have purchase the advertiser's
        product, 80% are considering a competitor brand and half of them will
        end up choosing the competitor's brand. Thus, 60% of switchers will buy
        the advertiser's brand, and 40% will buy a competitor's brand.
...
sales.params <- list(
    ...
    advertiser.demand.intercept = list(
        favorability = c(.014, 0, 0.2, 0.3, 0.9)),
    price = 80)
```
...
*[research.google.com](https://research.google.com)*.
